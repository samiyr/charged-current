...
struct SIDIS {
	...
	PerturbativeResult differential_cross_section_xQ2(const double z, const TRFKinematics kinematics) {
		...
		if (compute_differential_cross_section_directly) {
			return combine_integrals ? sidis.differential_cross_section_xQ2_direct_combined(z, kinematics) : sidis.differential_cross_section_xQ2_direct_separated(z, kinematics);
		} else {
			return combine_integrals ? sidis.differential_cross_section_xQ2_indirect_combined(z, kinematics) : sidis.differential_cross_section_xQ2_indirect_separated(z, kinematics);
		}
	}
	...
	void lepton_pair_cross_section_xy(const std::vector<double> x_bins, const std::vector<double> y_bins, const std::vector<double> E_beam_bins, const std::string filename) {
		const size_t x_step_count = x_bins.size();
			...
			for (size_t i = 0; i < x_step_count; i++) {
				for (size_t j = 0; j < E_beam_step_count; j++) {
					for (size_t k = 0; k < y_step_count; k++) {
						...
						const PerturbativeResult cross_section_xQ2 = combine_integrals ? sidis.lepton_pair_cross_section_xQ2_combined(kinematics) : sidis.lepton_pair_cross_section_xQ2_separated(kinematics);
						const double jacobian = SIDISFunctions::xy_jacobian(kinematics, process);
						const PerturbativeResult cross_section_xy = cross_section_xQ2 * jacobian;

						#pragma omp critical
						{
							file << x << ", " << y << ", " << E_beam << ", " << cross_section_xy.lo << ", " << cross_section_xy.nlo << std::endl;
							file.flush();

							calculated_values++;
							std::cout << "Calculated value " << calculated_values << " / " << x_step_count * y_step_count * E_beam_step_count << ": " << cross_section_xy.lo << ", " << cross_section_xy.nlo << " (x = " << x << ", y = " << y << ", s = " << kinematics.s << ", E_beam = " << E_beam << ", Q2 = " << kinematics.Q2 << ")" << std::endl;
						}
					}
				}
			}
