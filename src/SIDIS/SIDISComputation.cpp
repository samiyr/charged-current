...
class SIDISComputation {
	...
	PerturbativeQuantity compute_structure_function(
		...
		case StructureFunction::F2: return F2_combined(z, kinematics);
		case StructureFunction::FL: return FL_combined(z, kinematics);
		case StructureFunction::F3: return F3_combined(z, kinematics);
	...
	// 		nlo = nlo_coefficient * (xi_integral + xip_integral + xi_xip_integral + delta_integral);
	// 	}
