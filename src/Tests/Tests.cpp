...
TEST(SIDIS, QuarksGluonsScaleLogs) {
	...
	for (std::size_t i = 0; i < xz_values.size(); i++) {
		for (std::size_t j = 0; j < xz_values.size(); j++) {
			...
			#pragma omp critical
			{
				EXPECT_REL_NEAR(computed_value_F2, comparison_value_F2, 1e-3);
				EXPECT_REL_NEAR(computed_value_FL, comparison_value_FL, 1e-3);
				EXPECT_REL_NEAR(computed_value_xF3, comparison_value_xF3, 1e-3);
			}
		}
	}
}
