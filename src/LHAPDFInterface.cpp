#ifndef LHAPDF_INTERFACE_H
#define LHAPDF_INTERFACE_H
...
class LHAPDFInterface : public PDFInterface {
	public:
	...
	LHAPDFInterface(std::string _pdf_set_name, int _set_member_number = 0)
	...
	// LHAPDFInterface(const LHAPDFInterface &p) {
	...
	void evaluate(const double x, const double Q2) override {
		_pdf->xfxQ2(x, Q2, flavor_values);
	}
	double xf_evaluate(const FlavorType flavor, const double x, const double Q2) override {
		return _pdf->xfxQ2(flavor, x, Q2);
	}
	double xf(const FlavorType flavor) const override {
		return flavor_values[size_t(flavor + 6)];
	}
	double alpha_s(const double Q2) const override {
