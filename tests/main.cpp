...
TEST(SIDIS, OnlyQuarks) {
	const FunctionalFormInterface pdf = FunctionalFormInterface([](const FlavorType flavor, const double x, [[maybe_unused]] const double Q2) {
		if (Flavor::is_gluon(flavor)) { return 0.0; }
		const double f = double(flavor);
		return (2.0 * f * f - f + 1.0) * x * (1 - x);
	});

	const FunctionalFormInterface ff = FunctionalFormInterface([](const FlavorType flavor, const double x, [[maybe_unused]] const double Q2) {
		if (Flavor::is_gluon(flavor)) { return 0.0; }
		const double f = double(flavor);
		return (2.0 * f * f + f + 1.0) * x * x * (1 - x) * (1 - x);
	});

	const FragmentationConfiguration ff_config({ff}, {Decay()});
	...
	sidis.integration_parameters.cuba.maximum_relative_error = 1e-5;
	sidis.integration_parameters.cuba.maximum_evaluations = 50'000'000;
	...
	for (std::size_t i = 0; i < xz_values.size(); i++) {
		for (std::size_t j = 0; j < xz_values.size(); j++) {
			...
			const TRFKinematics kinematics = TRFKinematics::y_E_beam(x, 1.0, 1.0, 1.0, 0.0);

			const double computed_value_F2 = sidis.F2(
				z, kinematics, PerturbativeOrder::NLO, false, 0.0, 0.0, 
				pdf, ff_config, 
				ScaleDependence::trivial, ScaleDependence::trivial, ScaleDependence::trivial
			).nlo;
			const double computed_value_FL = sidis.FL(
				z, kinematics, PerturbativeOrder::NLO, false, 0.0, 0.0, 
				pdf, ff_config, 
				ScaleDependence::trivial, ScaleDependence::trivial, ScaleDependence::trivial
			).nlo;
			const double computed_value_xF3 = x * sidis.F3(
				z, kinematics, PerturbativeOrder::NLO, false, 0.0, 0.0, 
				pdf, ff_config, 
				ScaleDependence::trivial, ScaleDependence::trivial, ScaleDependence::trivial
			).nlo;
			...
			#pragma omp critical
			{
				EXPECT_REL_NEAR(computed_value_F2, comparison_value_F2, 1e-2);
